...
config = [
  {
    ...
    data_primary_key  = "anotherKey,id"
...
| config                        | A configuration object containing environment variables for the lambda. These relate to the path_value_filter (the path of the data to filter specific records to Redis) the data_primary_key (used as the group id of the underlying SQS FIFO) and the redis_hash_keys (used to extract a value from the data and create a distinct record) | `list(object)` | `[]`    |    no    |
